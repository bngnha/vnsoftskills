...
function initHighslideSmartResizer($addslideshow = 0) {
	...
	$doc->addScript('plugins/content/smartresizer/'.$paddpath.'js/highslide/highslide-with-gallery.packed.js');
		...
		if(!version_compare(JVERSION,'1.6.0','<'))
		{
			...
			if ($option == 'com_content')
			{
				...
				else
				{
					...
					else if(!$resize_image_intro && $resize_image_article)
					{
...
// echo "==================== ".$urlbase . " ======================";
			...
			{
				$onsite=0;
				if (substr($uhost, strlen($uhost)-1) == "/")
				{
					$uhost = substr($uhost,0, strlen($uhost)-1);
				}
			}
...
//echo $full_path. ' : '. $thumb_path . ' : '. $thethumb;
			...
			if (!file_exists($thumb_path))
			{
				...
				if (isset($awidth[1]))
				{
					$individ_blogwidth = trim($awidth[1]);
				}
				else
				{
					$individ_blogwidth="";
				...
				if (isset($aheight[1]))
				{
					$individ_blogheight = trim($aheight[1]);
				}
				else
				{
					$individ_blogheight="";
				...
				{
					if (!$is_blog  && ($individ_width || $individ_height))
					{
						// this is article or other
				...
				elseif (!$calcthumb_width  && $calcthumb_height)
				{
			...
			if ((int) $botParams->def( 'uselargethumb', '0'))
			{
				...
				if (!file_exists($lthumb_path))
				{
					...
					if ($image_width!=0 && $image_height!=0)
					{
						...
						else if (!$calcthumb_width  && $calcthumb_height)
						{
							$calcthumb_width = round($calcthumb_height * ($image_width/$image_height));
						}
			...
			if ($imgstyle)
			{
				if (strrpos($imgstyle,';') === (strlen($imgstyle)-1))
				{
					$adds = "";
				}
				else
